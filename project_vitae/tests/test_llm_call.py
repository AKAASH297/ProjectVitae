...
def _make_response_with_usage(input_t: int = 100, output_t: int = 50):
    ...
    response.text = "test"
    response.response_metadata = {"usage": {"input_tokens": input_t, "output_tokens": output_t}}
    return response
...
def test_retry_then_succeed(session_dir: Path, cost_guard: CostGuard, monkeypatch):
    ...
    llm = LLMCall(
        "test", cfg, session_dir, FakeOutput, cost_guard, retry_cfg=retry, config=_make_config()
    )
