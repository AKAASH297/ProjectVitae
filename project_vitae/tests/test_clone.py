...
def _make_state(urls: list[str] | None = None) -> SessionState:
    return SessionState(session_name="test-session", github_urls=urls or ["https://github.com/user/repo1"])
...
def test_clone_success(tmp_path: Path, monkeypatch):
    ...
    with patch("project_vitae.nodes.clone._check_github_size", return_value=1000), \
